...
copier \
        ...
        --copy-sql="SELECT {{.CopyColumns}} FROM {{.SrcTableName}} WHERE fires_at >='2022-04-01 00:00:00' and fires_at < '2022-05-01 00:00:00' AND name='SubscriptionMonthlyTimer' AND {{.DomainIDName}} >= \$1 AND {{.DomainIDName}} <= \$2{{if .CursorPredicate}} AND ({{.PrimaryKeys}}) >= ({{.PrimaryKeyPlaceholders}}){{end}} ORDER BY {{.PrimaryKeysAsc}} LIMIT {{.SrcQueryLimit}}" \
        \
        ...
        --dst-table-name=timers_bt_06_2022_bkup \
        \
        --column=id \
        --column=name \
        --column=type \
